...
import { authHeaders, ORDERS, ANALYTICS, STORES, CUSTOMERS, NOTIFS, LOGS, PROFILE, PLANS, SETTINGS } from '../lib/api';
...
export default function Dashboard() {
  ...
  const fetchAll = async () => {
    ...
    try {
      ...
      const [o, a, s, c, n, l, p, plans] = await Promise.all([
        ...
        safe(() => NOTIFS.list(),       []),
        safe(() => LOGS.list(20),       []),
        safe(() => PROFILE.get(),       null),
        safe(() => PLANS.list(),        []),
      ]);
      setOrders(Array.isArray(o) ? o : []);
      setAnalytics(a && !(a as any).error ? a : {});
      setStores(Array.isArray(s) ? s : []);
      setCustomers(Array.isArray(c) ? c : []);
      setNotifications(Array.isArray(n) ? n : []);
      setLogs(Array.isArray(l) ? l : []);
      if (p && typeof p === 'object') { setProfile(p); setAutoForward((p as any).auto_forward || false); }
      if (Array.isArray(plans)) {
        const map: any = {};
        (plans as any[]).forEach((pl: any) => { map[pl.plan_key] = pl; });
        setPlansList(map);
      }
  ...
  const handleLogout = async () => {
    ...
    if (token) {
      await fetch('/api/auth', { method: 'POST', headers: authHeaders(), body: JSON.stringify({ action: 'logout', token }) }).catch(() => {});
    }
  ...
  const toggleAutoForward = async (val: boolean) => {
    ...
    await Promise.all([
      fetch('/api/profiles', { method: 'PUT', headers: authHeaders(), body: JSON.stringify({ auto_forward: val }) }),
      fetch('/api/settings', { method: 'PUT', headers: authHeaders(), body: JSON.stringify({ auto_forward_global: String(val) }) }),
      fetch('/api/logs', { method: 'POST', headers: authHeaders(), body: JSON.stringify({ action: val ? 'Enabled auto-forward' : 'Disabled auto-forward', entity: 'settings', entity_id: 'auto_forward' }) }),
    ]).catch(() => {});
  };
  ...
  const markAllRead = async () => {
    await fetch('/api/notifications', { method: 'PUT', headers: authHeaders(), body: JSON.stringify({ user_id: user?.id || 'demo', mark_all: true }) });
    fetchAll();
  };
        ...
        <header className="flex items-center gap-3 px-5 h-16 bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 flex-shrink-0">
          ...
          <div className="flex items-center gap-2">
            ...
            <Tooltip>
              ...
              <TooltipContent>{autoForward ? 'Auto-forward ON' : 'Auto-forward OFF'}</TooltipContent>
            </Tooltip>
        ...
        <div className="lg:col-span-2 bg-white dark:bg-gray-900 rounded-2xl p-5 border border-gray-200 dark:border-gray-800">
          ...
          <ResponsiveContainer width="100%" height={220}>
            <AreaChart data={analytics?.revenueChart || []}>
              ...
              <RechartsTooltip contentStyle={{ background: '#1f2937', border: '1px solid #374151', borderRadius: '12px', color: '#fff', fontSize: 12 }} />
...
function OrdersTab({ orders, allOrders, statusFilter, setStatusFilter, searchQ, setSearchQ, updateOrderStatus, deleteOrder, fetchAll, formatCurrency, selectedOrders, setSelectedOrders, autoForward }: any) {
  ...
  const [newOrder, setNewOrder] = useState({ customer_name: '', customer_phone: '', wilaya: '', address: '', carrier: 'Yalidine', total: '', payment_method: 'COD', notes: '', product_name: '' });
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setShowAdd(false);
      setNewOrder({ customer_name: '', customer_phone: '', wilaya: '', address: '', carrier: 'Yalidine', total: '', payment_method: 'COD', notes: '', product_name: '' });
      toast.success(`Order ${order.order_number} created! ✅`);
      // Immediate refresh to show new order
      await fetchAll();
    } catch (err: any) {
  ...
  const bulkUpdate = async (status: string) => {
    ...
    await Promise.all(selectedOrders.map((id: string) =>
      fetch('/api/orders', { method: 'PUT', headers: authHeaders(), body: JSON.stringify({ id, status }) })
    ));
    setSelectedOrders([]);
    fetchAll();
  };
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <WilayaSelect
                ...
                onChange={(wilaya, carrier) => setNewOrder({...newOrder, wilaya, carrier})}
                required
              />
            </div>
            <div>
...
function StoresTab({ stores, fetchAll }: any) {
  ...
  const handleAdd = async (e: React.FormEvent) => {
    e.preventDefault();
    await fetch('/api/stores', { method: 'POST', headers: authHeaders(), body: JSON.stringify({ ...form, status: 'active', orders_count: 0 }) });
    setShowAdd(false);
...
function NotificationsTab({ notifications, markAllRead, fetchAll }: any) {
  ...
  const deleteNotif = async (id: string) => {
    await fetch('/api/notifications', { method: 'DELETE', headers: authHeaders(), body: JSON.stringify({ id }) });
    fetchAll();
  };
...
function SettingsTab({ user, profile, platformSettings, refreshSettings, autoForward, toggleAutoForward, fetchAll, plansList }: any) {
  ...
  const { lang, setLang, token } = useApp();
  ...
  useEffect(() => {
    if (profile) setProfileForm({ name: user?.name || '', company: profile.company || '', phone: profile.phone || '', wilaya: profile.wilaya || '' });
  }, [profile, user]);

  // Load devices
  useEffect(() => {
    if (section === 'security') {
      fetch('/api/admin/devices', { headers: authHeaders() }).then(r => r.json()).then(setDevices).catch(() => {});
    }
  }, [section]);
  ...
  const saveProfile = async () => {
    setSaving(true);
    try {
      await fetch('/api/profiles', { method: 'PUT', headers: authHeaders(), body: JSON.stringify(profileForm) });
      await fetchAll();
  ...
  const sections = [
    { id: 'profile',      label: t(lang, 'Profile'),      icon: UserCheck },
    { id: 'automation',   label: t(lang, 'Automation'),   icon: Zap },
    { id: 'carriers',     label: t(lang, 'Carriers'),     icon: Truck },
    { id: 'security',     label: t(lang, 'Security'),     icon: Shield },
    { id: 'notifications',label: t(lang, 'Notifications'),icon: Bell },
    { id: 'billing',      label: t(lang, 'Billing'),      icon: CreditCard },
  ];

  const curToken = token || localStorage.getItem('af_session_token') || '';

  return (
    <div style={{ display: 'flex', gap: 20 }}>
      {/* Settings sidebar */}
      <div style={{ width: 180, flexShrink: 0 }}>
        <div style={{ background: 'rgba(255,255,255,.03)', border: '1px solid rgba(255,255,255,.08)', borderRadius: 16, padding: 8, display: 'flex', flexDirection: 'column', gap: 2 }}>
          {sections.map(s => (
            <button key={s.id} onClick={() => setSection(s.id)} style={{
              ...
              background: section === s.id ? 'rgba(124,58,237,.25)' : 'transparent',
              color: section === s.id ? '#c4b5fd' : 'rgba(232,228,255,.5)', transition: 'all .15s',
            }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        {section === 'profile' && (
          <div>
            <h2 style={{ fontSize: 20, fontWeight: 700, color: '#e8e4ff', margin: '0 0 6px' }}>Profile</h2>
            <p style={{ fontSize: 13, color: 'rgba(232,228,255,.4)', margin: '0 0 24px' }}>Update your personal information.</p>
            <div style={{ background: 'rgba(255,255,255,.03)', border: '1px solid rgba(255,255,255,.08)', borderRadius: 14, padding: 24 }}>
              {/* Avatar */}
              <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 24, paddingBottom: 24, borderBottom: '1px solid rgba(255,255,255,.06)' }}>
                <div style={{ width: 72, height: 72, borderRadius: '50%', background: 'linear-gradient(135deg,#7c3aed,#a855f7)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 28, fontWeight: 800, color: '#fff', flexShrink: 0 }}>
                  {profile?.avatar_url ? <img src={profile.avatar_url} style={{ width: 72, height: 72, borderRadius: '50%', objectFit: 'cover' }} /> : (profileForm.name.charAt(0) || user?.email?.charAt(0) || '?').toUpperCase()}
                </div>
                <div>
                  <div style={{ fontWeight: 700, fontSize: 16, color: '#e8e4ff' }}>{profileForm.name || user?.email}</div>
                  <div style={{ fontSize: 12, color: 'rgba(232,228,255,.4)', marginTop: 3 }}>{user?.email} · {user?.plan || 'trial'}</div>
                  <div style={{ fontSize: 11, marginTop: 8, padding: '3px 10px', borderRadius: 100, background: 'rgba(124,58,237,.15)', color: '#a78bfa', display: 'inline-block', fontWeight: 700 }}>
                    {user?.role || 'owner'}
                  </div>
                </div>
              </div>
              {/* Form */}
              {[['name','Full Name','text'],['company','Company','text'],['phone','Phone','tel'],['wilaya','Wilaya','text']].map(([field, label, type]) => (
                <div key={field} style={{ marginBottom: 16 }}>
                  <div style={{ fontSize: 11, fontWeight: 700, color: 'rgba(232,228,255,.4)', textTransform: 'uppercase', letterSpacing: '.08em', marginBottom: 6 }}>{label}</div>
                  <input type={type} value={(profileForm as any)[field]} onChange={e => setProfileForm(p => ({ ...p, [field]: e.target.value }))}
                    style={{ width: '100%', padding: '10px 14px', background: 'rgba(255,255,255,.05)', border: '1px solid rgba(255,255,255,.1)', borderRadius: 10, color: '#e8e4ff', fontSize: 14, outline: 'none', boxSizing: 'border-box' }} />
                </div>
              ))}
              {/* Language */}
              <div style={{ marginBottom: 20 }}>
                <div style={{ fontSize: 11, fontWeight: 700, color: 'rgba(232,228,255,.4)', textTransform: 'uppercase', letterSpacing: '.08em', marginBottom: 6 }}>Language</div>
                <select value={lang} onChange={async e => { setLang(e.target.value as any); await fetch('/api/profiles/language', { method: 'PUT', headers: authHeaders(), body: JSON.stringify({ language: e.target.value }) }); }}
                  style={{ width: '100%', padding: '10px 14px', background: 'rgba(255,255,255,.07)', border: '1px solid rgba(255,255,255,.12)', borderRadius: 10, color: '#e8e4ff', fontSize: 14, cursor: 'pointer', outline: 'none' }}>
              ...
              <button onClick={saveProfile} disabled={saving}
                style={{ padding: '11px 24px', borderRadius: 11, border: 'none', cursor: 'pointer', fontWeight: 700, fontSize: 14, background: 'linear-gradient(135deg,#7c3aed,#a855f7)', color: '#fff', opacity: saving ? 0.7 : 1 }}>
        ...
        {section === 'automation' && <AutomationSettings token={curToken} />}
        {section === 'carriers' && <CarriersSection token={curToken} />}
        {section === 'security' && <SecuritySection token={curToken} profile={profile} devices={devices} onRefreshDevices={() => fetch('/api/admin/devices', { headers: authHeaders() }).then(r => r.json()).then(setDevices).catch(() => {})} />}
        {section === 'billing' && <BillingSection token={curToken} currentPlan={user?.plan || 'trial'} trialEnd={user?.trialEnd} />}
        {section === 'notifications' && (
          <div>
            <h2 style={{ fontSize: 20, fontWeight: 700, color: '#e8e4ff', margin: '0 0 6px' }}>Notifications</h2>
            <p style={{ fontSize: 13, color: 'rgba(232,228,255,.4)', margin: '0 0 24px' }}>Manage how you receive notifications.</p>
            <div style={{ background: 'rgba(255,255,255,.03)', border: '1px solid rgba(255,255,255,.08)', borderRadius: 14, padding: 24 }}>
              <div style={{ fontWeight: 700, fontSize: 15, color: '#e8e4ff', marginBottom: 8 }}>Push Notifications</div>
              <p style={{ fontSize: 13, color: 'rgba(232,228,255,.4)', margin: '0 0 16px', lineHeight: 1.6 }}>
                Receive push notifications even when the browser is closed. Works on desktop and mobile.
              </p>
              <PushNotificationButton token={curToken} />
            </div>
            {/* Auto-forward setting */}
            <div style={{ background: 'rgba(255,255,255,.03)', border: '1px solid rgba(255,255,255,.08)', borderRadius: 14, padding: 24, marginTop: 16 }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <div>
                  <div style={{ fontWeight: 700, fontSize: 15, color: '#e8e4ff' }}>Auto-forward Orders</div>
                  <div style={{ fontSize: 12, color: 'rgba(232,228,255,.4)', marginTop: 3 }}>Automatically confirm and assign carrier when order is created</div>
                </div>
                <Toggle checked={autoForward} onChange={toggleAutoForward} />
              </div>
            </div>
          </div>
        )}
